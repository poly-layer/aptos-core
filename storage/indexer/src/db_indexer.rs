...
impl DBIndexer {
    ...
    fn get_num_of_transactions(&self, version: Version) -> Result<u64> {
        ...
        let num_of_transaction = min((self.config.batch_size + 1) as u64, highest_version - version + 1) ;
        ...
        db_iter.try_for_each(|res| {
            ...
            if let Some(txn) = txn.try_as_signed_user_txn() {
                ...
                if self.config.enable_statekeys {
                    writeset.iter().for_each(|(state_key, _write_op)| {
                        batch
            ...
            if let Some(writeset_payload) = txn.try_as_genesis_txn() {
                if let Some(changeset) = writeset_payload.get_changeset() {
                    changeset
                        .write_set()
                        .into_iter()
                        .for_each(|(state_key, _value)| {
                            batch
                                .put::<StateKeysSchema>(state_key, &())
                                .expect("Failed to put state keys to a batch");
                        });
