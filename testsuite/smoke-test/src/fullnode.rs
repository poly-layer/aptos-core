...
async fn test_internal_indexer_with_fast_sync() {
    ...
    let vfn_storage_path = TempPath::new().path().to_path_buf();
    vfn_config.storage.set_data_dir(vfn_storage_path);

    vfn_config.state_sync.state_sync_driver.bootstrapping_mode =
        BootstrappingMode::DownloadLatestStates;
    enable_internal_indexer(&mut vfn_config);

    let vfn_peer_id = create_fullnode(vfn_config.clone(), &mut swarm).await;
    swarm.wait_for_all_nodes_to_catchup(Duration::from_secs(60)).await.unwrap();
    let vfn: &mut dyn aptos_forge::FullNode = swarm.full_node_mut(vfn_peer_id).unwrap();
    vfn.stop().await.unwrap();
    ...
    let aptos_db_dir = vfn_config
        ...
        .default_root_path()
        .to_owned();
    let aptos_db = AptosDB::new_for_test(aptos_db_dir.as_path());

    // assert state keys in the indexer db is same as the ones in the main db
    ...
    let main_db_iter = aptos_db
        ...
        .unwrap();
    let mut main_db_keys: Vec<StateKey> = main_db_iter.map(|iter| iter.unwrap().0).collect();
    let mut indexer_db_iter = internal_indexer_db.iter::<StateKeysSchema>().unwrap();
    indexer_db_iter.seek_to_first();
    let mut indexer_keys = indexer_db_iter
        .map(|iter| iter.unwrap().0)
        .collect::<Vec<StateKey>>();
    main_db_keys.sort();
    indexer_keys.sort();
    println!("Total state keys: {}", main_db_keys.len());
    assert_eq!(main_db_keys, indexer_keys);
}
